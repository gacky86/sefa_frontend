...
export const getFlashcardList = () => {
  return client.get<Flashcard[]>('/flashcards', {params: { only_mine: "onlyMine" }});
};
...
export const getCardToLearn = (flashcardId: number, learningMode: 'input' | 'output', lastCardId: number) => {
  return client.get(`/flashcards/${flashcardId}/card_to_learn`, { params: { learning_mode: learningMode, last_card_id: lastCardId } })
}
